...
const FeedList: FC<{}> = () => {
  ...
  useEffect(() => {
    if (post) {
      setMeta({
        meta: {
          "og:title": `Pubsky | View Thread`,
          charset: "utf-8",
          "og:description": `View Thread from ${post.actor_handle || ""}`,
          "og:author": post.actor_handle ? post.actor_handle : "",
          "og:image":
            post.images && post.images.length > 0 ? post.images[0].url : "",
          name: {
            keywords: "pubsky, skydb, skydb, skynet, skylink, decentralized",
          },
        },
      });
    }
