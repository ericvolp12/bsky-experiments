...
func NewHandlers(graph *graphfd.Graph) *Handlers {
	return &Handlers{
		graph: graph,
	}
}
...
func (h *Handlers) PostFollow(c echo.Context) error {
	...
	followRkey, err := syntax.ParseRecordKey(body.Rkey)
	if err != nil {
		return c.JSON(400, fmt.Sprintf("invalid follow rkey: %s", err))
	}

	actorUID := h.graph.AcquireDID(actorDid.String())
	targetUID := h.graph.AcquireDID(targetDid.String())
	h.graph.AddFollow(actorUID, targetUID, followRkey.String())
...
func (h *Handlers) PostFollows(c echo.Context) error {
	...
	for i, follow := range body.Follows {
		...
		_, err = syntax.ParseRecordKey(follow.Rkey)
		if err != nil {
			return c.JSON(400, fmt.Sprintf("invalid follow rkey[%d]: %s", i, err))
		}
	}
	...
	for _, follow := range body.Follows {
		...
		h.graph.AddFollow(actorUID, targetUID, follow.Rkey)
	}
